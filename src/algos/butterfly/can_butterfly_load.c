...
int can_butterfly(const t_data *data, const int nb, const t_lnk *lst)
{
    if (set_for(is_low, data, lst))
        return (is_low(data, nb, lst));
    else if (set_for(is_high, data, lst))
        return (is_high(data, nb, lst));
