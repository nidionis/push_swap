...
/*   Updated: 2025/04/02 17:05:14 by nidionis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

int	apply_set_and(int (*f_do)(t_data *d, int instr), t_data *data,
		t_instr_step *instr_step, int verbose)
{
	if (instr_step->instr == NO_INSTR)
		return (NO_INSTR);
	else if (instr_step->nb_instr == CANT_INSERT
		|| instr_step->nb_instr == SIZE_MAX || instr_step->nb_instr == 0)
