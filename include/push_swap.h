...
t_list					*ft_best_comb(t_data *d, int *instr_ls,
							int (*can_push)(t_data *), int max_cost);
t_list					*best_insert(t_data *d, int lst_instr[],
							int (*can_push)(t_data *), int max_cost);
int						get_steps(t_list *instr_step_node);
t_list					*init_instr_step_node(int instr, int nb_instr_init);
int						count_instr(t_data *data, int instr,
							int (*can_push)(t_data *data), int max);
int						get_shortestway(int target_rank, t_lnk *list);
int						is_sorted(t_lnk *lst);
int						ft_no_duplicate(t_lnk *lst);
t_list					*handle_best_comb(t_best_comb *best,
							t_list *instr_step_node, int instr, int nb_instr);
void					handle_else_case(t_data *d_copy,
							t_instr_step *first_intr_step, int instr,
							t_list *instr_step_node);
int						should_update(t_best_comb *best,
							t_list *instr_step_node, int nb_first_instr);
t_list					*best_insert_dir(t_data *d, int instr,
							int (*can_push)(t_data *), int max_cost);
void					handle_no_best_comb(t_list *first_intr_step_node,
							t_data *d_copy, int instr);
t_list					*best_insert_dir(t_data *d, int instr,
							int (*can_push)(t_data *), int max_cost);
...
void					data_update_r(t_data *data, t_lnk **lst_a,
							t_lnk **lst_b);
void					init_data(t_data *data, t_lnk **lst_a, t_lnk **lst_b);
void					print_data(t_data *d);
char					*ft_realloc_cat(char *line, char c, int *len);
char					*get_next_line(int fd);
int						apply_set_and(int (*f_do)(t_data *d, int instr),
							t_data *data, t_instr_step *instr_step,
							int verbose);
int						apply_best_comb_and(int (*f_do)(t_data *d, int instr),
							t_data *data, t_list *best_comb, int verbose);
int						instr_direction(int instr);
void					reach_rank_dir(t_lnk **lst, int rank, int instr,
							int verbose);
